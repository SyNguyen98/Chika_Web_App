...
class SignUp extends Component {
    ...
    render() {
        ...
        switchWifi.forEach((item, i) => {
            switchWifis.push(<ProductInput key={i} getFieldDecorator={getFieldDecorator} productName={SWITCH_WIFI}
                                           productNum={i}
                                           handleRemove={this.handleRemoveProduct} onChange={this.onChange}/>)
        });
        switchRf.forEach((item, i) => {
            switchRfs.push(<ProductInput key={i} getFieldDecorator={getFieldDecorator} productName={SWITCH_RF}
                                         productNum={i}
                                         handleRemove={this.handleRemoveProduct} onChange={this.onChange}/>)
        });
        moduleIr.forEach((item, i) => {
            moduleIrs.push(<ProductInput key={i} getFieldDecorator={getFieldDecorator} productName={MODULE_IR}
                                         productNum={i}
                                         handleRemove={this.handleRemoveProduct} onChange={this.onChange}/>)
        });
        homeCenter.forEach((item, i) => {
            homeCenters.push(<ProductInput key={i} getFieldDecorator={getFieldDecorator} productName={HOME_CENTER}
                                           productNum={i}
                                           handleRemove={this.handleRemoveProduct} onChange={this.onChange}/>)
        });
            ...
            <Form className="admin-user_add_form" onSubmit={this.handleSubmit} autoComplete="off">
                <div style={{display: 'flex'}}>
                    <div className="admin-user_add_form_info">
                        ...
                        <Button type="primary" htmlType="submit" size="large" className="admin-user_add_form_button">Tạo
                            tài khoản</Button>
                    </div>
                    ...
                    <div className="admin-user_add_form_product">
                        ...
                        {products.includes(SWITCH_WIFI) ? (
                            <div className="admin-user_add_form_product_type">
                                <p>Công tắc Wifi</p>
                                {switchWifis}
                                <Button type="dashed" onClick={() => this.handleAddProduct(SWITCH_WIFI)}><Icon
                                    type="plus"/>Thêm</Button>
                            </div>
                        ) : null}
                        ...
                        {products.includes(SWITCH_RF) ? (
                            <div className="admin-user_add_form_product_type">
                                <p>Công tắc Rf</p>
                                {switchRfs}
                                <Button type="dashed" onClick={() => this.handleAddProduct(SWITCH_RF)}><Icon
                                    type="plus"/>Thêm</Button>
                            </div>
                        ) : null}
                        ...
                        {products.includes(MODULE_IR) ? (
                            <div className="admin-user_add_form_product_type">
                                ...
                                {moduleIrs}
                                <Button type="dashed" onClick={() => this.handleAddProduct(MODULE_IR)}><Icon
                                    type="plus"/>Thêm</Button>
                            </div>
                        ) : null}
                        ...
                        {products.includes(HOME_CENTER) ? (
                            <div className="admin-user_add_form_product_type">
                                ...
                                {homeCenters}
                                <Button type="dashed" onClick={() => this.handleAddProduct(HOME_CENTER)}><Icon
                                    type="plus"/>Thêm</Button>
                            </div>
                        ) : null}
                ...
                <div style={{display: 'flex', marginBottom: '10px'}}>
                    ...
                    {productNum > 0 ? (
                        <Icon type="close-circle"
                              style={{marginTop: '7px', marginLeft: '10px', fontSize: '20px', cursor: 'pointer'}}
