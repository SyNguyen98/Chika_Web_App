...
import { Menu, Card, Icon, Row, Col } from 'antd';

import '../../../styles/guest/supporting/Document.css';
...
class Document extends Component {
  ...
  handleChangeDocument = (event, key) => {
    this.setState({
      referece: key
    });
  }

  handleOpenPdf = (event, path) => {
    window.open(path);
  }
  ...
  render() {
    var documentContent;
    if (this.state.referece === '1') {
      documentContent = (<Reference/>)
    } else if (this.state.referece === '2') {
      ...
      <div className="document">
        ...
        <div className="document_body">
          <Menu className="document_body_menu"
                ...
                mode="inline">
            <Menu.Item className="document_body_menu_item" key="1" onClick={(event) => this.handleChangeDocument(event, '1')}>
              <p>Tài liệu hướng dẫn</p>
            </Menu.Item>
            <Menu.Divider/>
            <Menu.Item className="document_body_menu_item" key="2" onClick={(event) => this.handleChangeDocument(event, '2')}>
              <p>Video hướng dẫn</p>
            </Menu.Item>
            <Menu.Divider/>
            <Menu.Item className="document_body_menu_item" key="3" onClick={(event) => this.handleOpenPdf(event, '/pdf/bang-gia.pdf')}>
    ...
    return(
      <div className="document_body_content">
        ...
        <Card className="document_body_content_card" title="Hướng dẫn lắp đặt công tắc cảm ứng Chika">
          <p>Được thiết kế phù hợp với đa số các ổ điện tiêu chuẩn hiện nay trên thị trường, công tắc Chika cực kỳ tiện lợi không phải đục đẽo tường hay khoan tường nhiều,
            không làm ảnh hưởng gì đến thẩm mỹ, thiết kế của ngôi nhà. Tuy nhiên, sau đây chúng tôi đưa ra hướng dẫn lắp đặt cơ bản công tắc thông minh Chika để khác hàng
